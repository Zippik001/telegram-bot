...
SILENCE_QUESTIONS = [
    ...
    "🤖 Если бы эта неделя была фильмом — какого жанра? Комедия, триллер, документалка?",
    "🤖 Что нового узнали или попробовали за последние дни? Даже мелочи считаются.",
    "🤖 Какая музыка сейчас в наушниках? Или тишина? Тоже ответ.",
    "🤖 Оцените своё настроение от 1 до 10. Объяснять не обязательно — но интересно.",
    "🤖 Если бы завтра был выходной — как бы провели день идеально?",
    "🤖 Какая была лучшая покупка за последний месяц? Или худшая — тоже принимается.",
    "🤖 Признайтесь: что откладываете уже давно и всё никак не сделаете? 😅",
    "🤖 Какое место в Братиславе стоит посетить но вы ещё не были?",
    "🤖 Порекомендуйте что-нибудь: фильм, сериал, книгу, место, блюдо — что угодно.",
    "🤖 Если бы вы могли изменить одну вещь в своём районе/городе — что бы это было?",
    "🤖 С кем из этой компании вы дольше всего не виделись лично? Может пора исправить? 👀",
    "🤖 Какая погода для вас идеальная? И где бы вы её провели?",
    "🤖 Что вас удивило на этой неделе? Хорошо или плохо — не важно.",
    "🤖 Один вопрос: как вы? Не «нормально» — а реально как?",
]
...
async def sched_check_silence(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Не повторювати питання яке вже задавалось останнє
    used = context.bot_data.get(f"last_silence_q_{chat_id}", "")
    available = [q for q in SILENCE_QUESTIONS if q != used]
    question = random.choice(available) if available else random.choice(SILENCE_QUESTIONS)
    context.bot_data[f"last_silence_q_{chat_id}"] = question
    try:
